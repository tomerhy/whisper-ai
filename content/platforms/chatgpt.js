...
(function() {
  ...
  function getTextareaContent(textarea) {
    if (!textarea) return '';
    return textarea.tagName === 'TEXTAREA' ? textarea.value : (textarea.innerText || '');
  }
  ...
  function setupTextareaListeners(textarea) {
    ...
    textarea.addEventListener('blur', () => {
      setTimeout(() => {
        if (getTextareaContent(textarea).length <= 10) hideQuickEnhanceButton();
      }, 200);
    });
  ...
  // Quick enhance - analyze and show result immediately
  ...
  // Result HTML with AIM breakdown
      ...
      <div class="whisper-content">
        ...
        <div class="whisper-aim">
          ...
          <div class="whisper-aim-grid">
            <div class="whisper-aim-item">
              <span class="whisper-aim-label">🎭 Actor</span>
              <span class="whisper-aim-value">${aimBreakdown?.actor || 'Expert'}</span>
            </div>
            <div class="whisper-aim-item">
              <span class="whisper-aim-label">📥 Input</span>
              <span class="whisper-aim-value">${aimBreakdown?.input || 'Request'}</span>
            </div>
            <div class="whisper-aim-item">
              <span class="whisper-aim-label">🎯 Mission</span>
              <span class="whisper-aim-value">${aimBreakdown?.mission || 'Help'}</span>
            </div>
          </div>
        </div>
        ...
        <div class="whisper-section">
          <div class="whisper-section-header">
            <span class="whisper-section-title">Sharpened Prompt</span>
          </div>
          <div class="whisper-prompt-box enhanced">${escapeHtml(enhancedPrompt)}</div>
        </div>
        
        <!-- What was added -->
        <div class="whisper-added">
          <span class="whisper-added-label">Added:</span>
          <span class="whisper-added-text">${aimBreakdown?.guidance || 'Clearer direction'}</span>
        </div>
  ...
  function saveToHistory() {
    ...
    try {
      chrome.runtime.sendMessage({
        action: 'saveHistory',
        data: { original: currentPrompt, enhanced: enhancedPrompt, platform: PLATFORM, date: new Date().toISOString() }
      });
    } catch (e) {}
  }
      ...
      chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        try {
          if (request.action === 'insertTemplate') {
            const textarea = getTextarea();
            if (textarea) { setTextareaContent(textarea, request.template); textarea.focus(); showToast('Inserted!', 'success'); }
            sendResponse({ success: true });
          }
          if (request.action === 'enhancePrompt') {
            if (request.userProfile) userProfile = request.userProfile;
            quickEnhance();
            sendResponse({ success: true });
          }
